...
Let us consider an area $A \subset \rsqr$ that we aim to cover with a number of mobile sensors. % will utilize many tools from various fields to achieve coverage over area $A \subset \rsqr$, under area and formation constraints. 
%The solution will be based on configurations (which are like the concept of frameworks). Given a set of sensors $S = \left\{s_1 \ldots s_n\right\}$ located in positions $p_i \in \mathbb{R}^2$ (for $i=1,\ldots,n$) at time $t$, a configuration $c(t)$ is stacking the sensor positions,
%\begin{equation}
%c\left(t\right) = \bmat{
%p_{1}^{T}\left(t\right)&\cdots&p_{n}^{T}\left(t\right)}^{T}\in\mathbb{R}^{2n}.
%\end{equation}
%Each sensor $i$ has a coverage radius $R$ (assuming all sensors are identical), so it can cover a disk $D\left(p_{i}\left(t\right), R\right)\subset \rsqr$, centred at $p_{i}\left(t\right)$, 
%\begin{equation}
%D\left(p_{i}\left(t\right),R\right) = D_{i}\left(t\right) = \{x\in\rsqr \mid\norm{x - p_{i}\left(t\right)}_{2} \leq R\}.
%\end{equation}
%The set of all possible configurations is defined as $\mathtt{C_{a}} = \left\{\conp{i}\right\} \in \mathbb{R}^{2r}$.
%Where $c_{i}(t)$ is the $i$-th configuration of the configurations set, and the total area that the $i$-th configuration can cover is %$A_{cov}(c_{i}(t))\subset \rsqr$,
%\begin{equation}\label{configuration set coverage}
%A_{cov}\left(c_{i}(t)\right) = \cup{_i}D_{i}\left(t\right)\subset \rsqr.
%\end{equation}
%Each configuration can be formed under some constraint. The constraints that will be dealt in this research are:
%We also define a \emph{Coverage Constraint}: A given area inside the area $A_{m} \subset A$ must be covered always (e.g. ground station). This condition, named \emph{Area Constraint Condition}, defines a set of possible configurations $\mathtt{C_{acc}} =  \left\{ \conp{i} \mid  A_{m} \subset A_{cov}\left(\conp{i}\right)  \right\}$.
...
%\textcolor{red}{(you have not actually clearly stated what the problem is!  And I think we should be very clear what we are doing - we have a 2-step process here, each one should be defined as a separate problem)
%\begin{problem}
