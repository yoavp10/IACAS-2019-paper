...
Sometimes the agents topology should be constrained. We refer to this problem as the \emph{formation control} problem \cite{Oh2015}. The constraints can be achieved using few methods \cite{Oh2015, Basiri2009}:
\begin{itemize}
...
\end{itemize}
In this chapter, the \emph{Distance-based} formation control will be in use.
...
\begin{proof}
    First of all, let us show that $P = C_{V}$ is an equilibrium point. Looking on (\ref{eq:Combined Controller}), it's easy to see that when $\norm{p_{i} - p_{j}}^{2} = d_{ij}^{2}$, $P = C_{V}$ is an equilibrium point. \br
